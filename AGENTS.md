...
- One reused Codex session per review cycle: the first pass is the `exec review` above (note the session id it prints); for re-reviews after fixing findings, resume that same session instead of a fresh review — `codex --dangerously-bypass-approvals-and-sandbox exec resume --last "<new commits address findings X/Y; re-review HEAD>"` (or `exec resume <session_id> "…"`). Resume is cwd-filtered, so run it from the same repo; reusing the session keeps the reviewer's prior context cached (faster, cheaper, remembers what it flagged).
...
- If the fresh review pass posts any actionable finding, fix it, validate, push, re-read thread-aware state, resolve only addressed threads, and resume the same Codex session (`exec resume`) for the next pass. Repeat until the current HEAD has an explicit no-findings result posted on the PR.
